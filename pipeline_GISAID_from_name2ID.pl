...
my %opts;
getopts('i:j:o:', \%opts);
my $input_fasta_sequence = $opts{'i'} or die "use: $0 -i input_fasta_sequence -j input_name2id -o output_file_name\n";
my $input_name2id = $opts{'j'} or die "use: $0 -i input_fasta_sequence -j input_name2id -o output_file_name\n";
my $output_file_name = $opts{'o'} or die "use: $0 -i input_fasta_sequence -j input_name2id -o output_file_name\n";
...
print "Step1, extract raw variations from fasta sequence file\n";
$options = "perl alignment2varients_no_screen_linux.pl -i $input_fasta_sequence -j $input_name2id -r $input_reference_genome -o $output_file_name";
print "$options\n\n";
system $options;

print "Step2, screening raw variations\n";
$options = "perl screen_summzrize_data_del_int.pl -i $output_file_name.raw_variants.for_each.all.txt -o $output_file_name";
print "$options\n\n";
system $options;

print "Step3, map snps to cds\n";
$options = "perl MapProtein_SARS2_aa_RNA_nsp_indel.pl -i $output_file_name.screen.ID.sum.txt -o $output_file_name";
print "$options\n\n";
system $options;

print "Step4, move genome-wide snps, codons and amino acid variations into genomic_variation folder\n";
...
print "step5, generate long tables for genomic variations";
$options = "perl variation_unique_ID_sum.pl -i $genomic_variation_folder";
print "$options\n\n";
system $options;

print "step6, calculate quality statistics of each sequence";
