...
my %opts;
getopts('i:j:o:', \%opts);
my $input_fasta_sequence = $opts{'i'} or die "use: $0 -i input_fasta_sequence -j input_metadata -o output_file_name\n";
my $input_metadata = $opts{'j'} or die "use: $0 -i input_fasta_sequence -j input_metadata -o output_file_name\n";
my $output_file_name = $opts{'o'} or die "use: $0 -i input_fasta_sequence -j input_metadata -o output_file_name\n";
...
if (!(-e $input_fasta_sequence)){
	print "Could not find input_fasta_sequence: $input_fasta_sequence, please check...\nExit...\n";
	exit();
} else {
...
if (!(-e $input_metadata)){
	print "Could not find input_metadata: $input_metadata, please check...\nExit...\n";
	exit();
} else {
...
$options = "perl name2ID.pl -i $input_metadata -o name2ID";
print "$options\n\n";
system $options;
